...
class APAClassifier(APABaseModel):
    """
    Base class for classical ML classifiers (e.g., Naive Bayes, Random Forest, Logistic Regression).
    Implements common training and prediction logic.
    """
    def train(self, X_train, y_train):
        """Train the underlying sklearn model."""
        # Drop 'record_id' column and fit the model
        self.model.fit(X_train.drop(columns=['record_id']), y_train['label'].to_list())

    def predict_next(self, novel_data):
        """Return the record_id of the most confident positive prediction."""
        # Get positive class probabilities for each instance
        positive_probs = self.get_positive_class_probs(self.model.predict_proba(novel_data.drop(columns=['record_id'])))
        # Find the index of the highest probability
        best_idx = np.argmax(positive_probs)
        # Return the corresponding record_id
        return novel_data['record_id'].tolist()[best_idx]

    def predict(self, novel_data):
        """Return binary predictions for all instances in novel_data."""
        tau = 0.5  # Threshold for positive class
        # Get positive class probabilities
        positive_probs = self.get_positive_class_probs(self.model.predict_proba(novel_data.drop(columns=['record_id'])))
        # Convert probabilities to binary predictions
        predictions = [1 if proba > tau else 0 for proba in positive_probs]
        # Add predictions to DataFrame
        prediction_df = novel_data.copy()
        prediction_df['prediction'] = predictions
        return prediction_df[['record_id', 'prediction']]

    def get_positive_class_probs(self, probas):
        """
        Extract the probability of the positive class (label=1) from the model's output.
        Handles cases where class labels may be missing or binary.
        """
        classes = self.model.classes_
        if 1 in classes:
            # Find index of positive class
            pos_idx = np.where(classes == 1)[0][0]
            positive_probs = probas[:, pos_idx]
        else:
            # Positive class (1) missing — fallback: assume binary and infer from negative
            if 0 in classes and len(classes) == 2:
                neg_idx = np.where(classes == 0)[0][0]
                positive_probs = 1.0 - probas[:, neg_idx]
            else:
                # Raise error if not binary
                raise ValueError(f"Model classes {classes} do not include positive class (1), or are not binary.")
        return positive_probs


class NaiveBayes(APAClassifier):
    """Naive Bayes classifier for text data."""
    def __init__(self):
        self.model = MultinomialNB()


class RandomForest(APAClassifier):
    """Random Forest classifier for tabular/text data."""
    def __init__(self):
        self.model = RandomForestClassifier(n_estimators=100, random_state=42)


class LogisticRegression(APAClassifier):
    """Logistic Regression classifier for tabular/text data."""
    def __init__(self):
        self.model = LR()
...
class ModelConverter:
    ...
    def convert(model_str,api_key=None,prompt=None):
        ...
        if 'bayes' in model_str_lower or 'naive' in model_str_lower:
            return NaiveBayes()
        elif 'forest' in model_str_lower or 'random' in model_str_lower:
            return RandomForest()
        elif 'logistic' in model_str_lower or 'regression' in model_str_lower:
            return LogisticRegression()
        elif 'hu' in model_str_lower and '1' in model_str_lower:
